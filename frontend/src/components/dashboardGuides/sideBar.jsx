...
export default function Sidebar() {
  ...
  return (
    <div className={`${open ? sidebarOpenClass : sidebarClosedClass} duration-300 h-[800px] -top-20 bg-primary p-3 flex flex-col relative`}>
      <div className={`absolute cursor-pointer top-40 right-[-20px] bg-primary rounded-full ${!open && "rotate-180"}`} onClick={() => setOpen(!open)}>
        <IoIosArrowDropleftCircle size={40} />
      </div>
      <div className={`rounded-full py-[100px] ml-12 w-[92px] h-[92px] items-center ${!open && "scale-0"}`}>
        <div className={`bg-white rounded-full ml-4 lg:ml-0 ${open ? 'w-[72px] h-[72px] lg:h-[92px] lg:w-[92px]' : ''}`}>
      ...
      <div className="flex flex-col">
        ...
        {error && <div className='text-white flex gap-2'>{"login before !"}<FiAlertTriangle size={20} color='red'/></div>}
        {guideInfo && (
          <>
