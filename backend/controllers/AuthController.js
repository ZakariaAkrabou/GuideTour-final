...
exports.loginUser = async (req, res) => {
    try {
        ...
        const token = JWT.sign({userid: user._id}, 'GAHDYSB', {expiresIn: '1d'})
...
exports.emailConfirm = async (req, res) => {
    try {
        const userToken = await User.findOne({ confirmationToken: req.params.token });
        if (!userToken) {
            return res.status(404).json({ message: "Token not found" });
        }
        // Update user's isConfirmed status
        await User.updateOne({ _id: userToken._id }, { $set: { isConfirmed: true }, $unset: { confirmationToken: 1 } });
