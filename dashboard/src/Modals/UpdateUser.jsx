...
function UpdateUser({ closeModal, userId }) {
  ...
  useEffect(() => {
    if (userId) {
      axios.get(`http://localhost:4000/api/admin/guide/${userId}`)
        .then(response => {
...
console.log(userId);
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:4000/api/admin/approval/${userId}/status`, formData); // Use correct URL with userId
      closeModal();
    } catch (error) {
